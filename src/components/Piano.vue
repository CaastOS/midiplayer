<template>
  <div id="keyboard">
    <div id="keys">
      <div v-for="note in notes" :key="note" :class="note.type" :id="note.note" class="key">
        <!-- keyboard creation loop -->
        {{ note.key }}
      </div>
    </div>
  </div>
...
export default {
  ...
  mounted() {
    ...
    for (let i = 0; i < notes.length; i++) {
      player[i] = () => {
        // array of arrow functions where each index plays a note
      ...
      window.addEventListener("keydown", function (event) {
        if (event.key.toLowerCase() == notes[i].key.toLowerCase() && !event.repeat)
          return player[i](); // emit a note when the relative key on physical keyboard is pressed
      });
    }
  },
};
